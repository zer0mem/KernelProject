...
static void print_insn_detail(csh ud, cs_mode mode, cs_insn *ins)
{
	...
	if ((mode & CS_MODE_16) == 0) {
		...
		if (x86->sib_index != X86_REG_INVALID)
			DbgPrint("\t\tsib_index: %s\n", cs_reg_name(handle, x86->sib_index));
		if (x86->sib_scale != 0)
			DbgPrint("\t\tsib_scale: %d\n", x86->sib_scale);
	}

	count = cs_op_count(ud, ins, X86_OP_IMM);
	if (count) {
		DbgPrint("\timm_count: %u\n", count);
		for (i = 1; i < count + 1; i++) {
			int index = cs_op_index(ud, ins, X86_OP_IMM, i);
			DbgPrint("\t\timms[%u]: 0x%"PRIx64 "\n", i, x86->operands[index].imm);
		}
	}
	...
	for (i = 0; i < x86->op_count; i++) {
		...
		switch((int)op->type) {
			case X86_OP_REG:
				DbgPrint("\t\toperands[%u].type: REG = %s\n", i, cs_reg_name(handle, op->reg));
				break;
			case X86_OP_IMM:
				DbgPrint("\t\toperands[%u].type: IMM = 0x%"PRIx64 "\n", i, op->imm);
				break;
			case X86_OP_FP:
				DbgPrint("\t\toperands[%u].type: FP = %f\n", i, op->fp);
				break;
			case X86_OP_MEM:
				DbgPrint("\t\toperands[%u].type: MEM\n", i);
				if (op->mem.base != 0)
					DbgPrint("\t\t\toperands[%u].mem.base: REG = %s\n", i, cs_reg_name(handle, op->mem.base));
				if (op->mem.index != 0)
					DbgPrint("\t\t\toperands[%u].mem.index: REG = %s\n", i, cs_reg_name(handle, op->mem.index));
				if (op->mem.scale != 1)
					DbgPrint("\t\t\toperands[%u].mem.scale: %u\n", i, op->mem.scale);
				if (op->mem.disp != 0)
					DbgPrint("\t\t\toperands[%u].mem.disp: 0x%" PRIx64 "\n", i, op->mem.disp);
				break;
			default:
				break;
		}
...
//#define X86_CODE32 "\xf3\xaa"		// rep stosb
	...
	for (i = 0; i < sizeof(platforms)/sizeof(platforms[0]); i++) {
		...
		if (err) {
			DbgPrint("Failed on cs_open() with error returned: %u\n", err);
			continue;
		}
		...
		if (count) {
			...
			DbgPrint("Platform: %s\n", platforms[i].comment);
			print_string_hex("Code:", platforms[i].code, platforms[i].size);
			DbgPrint("ERROR: Failed to disasm given code!\n");
		}
